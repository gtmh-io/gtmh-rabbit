...
");
    ...
    private InterfaceDefn ? ParseTargetInterface(SemanticModel semanticModel, InterfaceDeclarationSyntax ifaceDeclaration)
    {
      ...
      return new InterfaceDefn(ifaceName, ns, usings, methods);
    }
    ...
    public static Guid Identify(string a_Value, params string[] a_VA)
    {
      byte[] hash;
      using (var alg = System.Security.Cryptography.SHA1.Create())
      {
        byte[] buf;
				...
				{
					buf = Encoding.UTF8.GetBytes(v);
					alg.TransformBlock(buf, 0, buf.Length, null, 0);
				};
				add_hash(a_Value);
				for (int i = 0; i != a_VA.Length; ++i)
				{
					add_hash(a_VA[i]);
				}
        buf = Encoding.UTF8.GetBytes("GTMH");  // have a fixed final block
        alg.TransformFinalBlock(buf, 0, buf.Length);
        hash = alg.Hash;
      }
